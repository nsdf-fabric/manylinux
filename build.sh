...
DOCKER_PREFIX=${DOCKER_PREFIX:-quay.io/pypa} 


BUILD_ARGS_COMMON="
	--build-arg POLICY --build-arg PLATFORM --build-arg BASEIMAGE
	--build-arg DEVTOOLSET_ROOTPATH --build-arg PREPEND_PATH --build-arg LD_LIBRARY_PATH_ARG
	--rm -t $DOCKER_PREFIX/${POLICY}_${PLATFORM}:${COMMIT_SHA}
...
if [ "${MANYLINUX_BUILD_FRONTEND}" == "docker" ]; then
	...
	buildctl build \
		...
		--output type=docker,name=$DOCKER_PREFIX/${POLICY}_${PLATFORM}:${COMMIT_SHA} | docker load
else
	echo "Unsupported build frontend: '${MANYLINUX_BUILD_FRONTEND}'"
	exit 1
fi

docker run --rm -v $(pwd)/tests:/tests:ro $DOCKER_PREFIX/${POLICY}_${PLATFORM}:${COMMIT_SHA} /tests/run_tests.sh
